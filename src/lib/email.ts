...
/**
 * Send a single attendee their own QR ticket — used when the buyer assigns or
 * swaps an attendee from the dashboard. No-op when Resend is unconfigured.
 */
export async function sendAttendeeTicket(opts: {
  to: string;
  ...
  <div style="background:#fff6fa;padding:24px 0;font-family:Arial,Helvetica,sans-serif">
    <div style="max-width:600px;margin:0 auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #f0e0e8">
      ...
      <div style="padding:26px 28px;text-align:center">
        <p style="margin:0 0 6px;color:#171114">Hi ${escapeHtml(opts.attendeeName || "there")},</p>
        <p style="margin:0 0 20px;color:#4b3f45;line-height:1.6">
          Here's your ticket — show this QR code at the door. ${escapeHtml(opts.ticketTypeName)}.
        </p>
        <img src="cid:qr-${opts.qrToken}" width="180" height="180" alt="QR ticket" style="display:block;margin:0 auto;border:1px solid #f0e0e8;border-radius:12px;padding:8px" />
        <div style="color:#b3a6ac;font-size:11px;margin-top:8px;letter-spacing:.04em">Ticket ${escapeHtml(opts.qrToken.slice(0, 8).toUpperCase())} · Order ${escapeHtml(opts.orderNumber)}</div>
      </div>
      <div style="padding:18px 28px;border-top:1px solid #f0e0e8;color:#b3a6ac;font-size:12px;line-height:1.6">
        Sat 17 &amp; Sun 18 April 2027 · Grand Pavilion, Rosehill Gardens, Sydney<br/>
        Dance Teacher Expo · ABN 17 611 514 580
  ...
  try {
    await resend.emails.send({
      ...
      to: opts.to,
      ...(replyTo ? { replyTo } : {}),
      subject: `Your DTE 2027 ticket`,
      html,
      attachments: [
        {
          filename: `ticket-${opts.qrToken.slice(0, 8)}.png`,
          content: await qrPngBuffer(opts.qrToken),
          contentId: `qr-${opts.qrToken}`,
        },
      ],
    });
    return { sent: true };
  } catch (e) {
    console.error("[email] attendee ticket send failed", e);
    return { sent: false, reason: "send error" };
  }
}
